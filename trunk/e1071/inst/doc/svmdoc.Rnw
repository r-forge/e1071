...
%\VignetteIndexEntry{Support Vector Machines---the Interface to libsvm
%in package e1071}
%\VignetteDepends{e1071,mlbench,rpart,xtable}
...
Machine Learning Databases} (available in package \pkg{mlbench}) for
...
ozone data, also contained in \pkg{mlbench}. Again, we split the data
...
crossprod(svm.pred - testset[,4]) / length(testindex)

## rpart
rpart.model <- rpart(V4 ~ ., data = trainset)
rpart.pred  <- predict(rpart.model, testset[,-4])
crossprod(rpart.pred - testset[,4]) / length(testindex)
@ 
...
for (i in 1:reps) {
  ...
  svm.model <- svm(V4 ~ ., data = trainset, cost = 1000, gamma = 0.0001)
  svm.pred  <- predict(svm.model, testset[,-4])
  sv.res[i] <- crossprod(svm.pred - testset[,4]) / length(testindex)
  
  ## rpart
  rpart.model <- rpart(V4 ~ ., data = trainset)
  rpart.pred  <- predict(rpart.model, testset[,-4])
  rp.res[i] <- crossprod(rpart.pred - testset[,4]) / length(testindex)
}
